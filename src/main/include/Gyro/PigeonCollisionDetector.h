...
class PigeonCollisionDetector : public CollisionDetector
{
public:
    explicit PigeonCollisionDetector(std::shared_ptr<PigeonIMU> imu, double threshold_ = 1.0) : imu(imu), threshold(threshold_) {}
...
private:
    const std::shared_ptr<PigeonIMU> imu;
